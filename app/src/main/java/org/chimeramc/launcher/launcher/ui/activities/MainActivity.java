...
import org.chimeramc.launcher.core.news.ChangelogManager;
import org.chimeramc.launcher.core.versions.GameVersion;
import org.chimeramc.launcher.core.versions.VersionManager;
import org.chimeramc.launcher.databinding.ActivityMainBinding;
...
import org.chimeramc.launcher.ui.dialogs.PlayStoreValidationDialog;
import org.chimeramc.launcher.ui.views.MainViewModel;
...
import org.chimeramc.launcher.util.InstanceShortcutManager;
...
import org.chimeramc.launcher.util.PlayStoreValidator;
import org.chimeramc.launcher.util.ResourcepackHandler;
import org.chimeramc.launcher.util.StorageMigrationManager;
import org.chimeramc.launcher.util.StorageMigrationService;
import org.chimeramc.launcher.util.UIHelper;
import org.chimeramc.launcher.core.content.ContentManager;
...
import okhttp3.OkHttpClient;
 ...
 import org.chimeramc.launcher.ui.dialogs.LoadingDialog;
 import org.chimeramc.launcher.util.AccountTextUtils;
 import org.chimeramc.launcher.util.DialogUtils;

 import static org.chimeramc.launcher.core.minecraft.MinecraftProcessRestarterKt.ACTION_MAIN_ACTIVITY_FIRST_DRAWN;
 import static org.chimeramc.launcher.core.minecraft.MinecraftProcessRestarterKt.EXTRA_CLOSE_RESTART_ACTIVITY_ON_FIRST_DRAW;
 ...
 public class MainActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isInstanceShortcutIntent()) {
            new GithubReleaseUpdater(this, "LiteLDev", "ChimeraLauncher", permissionResultLauncher).checkUpdateOnLaunch();
        }
        showEulaIfNeeded();
     ...
     private void setupOnBackPressedCallback() {
         onBackPressedCallback = new OnBackPressedCallback(true) {
             ...
             public void handleOnBackPressed() {
                 org.chimeramc.launcher.ui.dialogs.CustomAlertDialog exitDialog = new org.chimeramc.launcher.ui.dialogs.CustomAlertDialog(MainActivity.this);
                 exitDialog.setTitleText(getString(org.chimeramc.launcher.R.string.dialog_title_exit_app))
                         .setMessage(getString(org.chimeramc.launcher.R.string.dialog_message_exit_app))
                         .setPositiveButton(getString(org.chimeramc.launcher.R.string.dialog_positive_exit), v -> {
                             exitDialog.dismissImmediately();
                             finishAffinity();
                         })
                         .setNegativeButton(getString(org.chimeramc.launcher.R.string.dialog_negative_cancel), null)
                         .show();
             }
         };
    ...
    private void applyPersonalization() {
        ...
        float animSpeed = pm.getAnimationSpeed();
        DynamicAnim.setGlobalSpeedMultiplier(animSpeed);
        
        // Apply UI Transparency
        int transparency = pm.getUiTransparency();
        ...
        // Apply Card Rounding
        int rounding = pm.getCardRounding();
        applyCardRounding(rounding);
        ...
        // Apply Glow Effects
        boolean glow = pm.isEnableGlowEffects();
        applyGlowEffects(glow);
        
        // Apply Compact Mode
        boolean compact = pm.isCompactMode();
        applyCompactMode(compact);
        
        // Apply Show Animations toggle
        boolean showAnims = pm.isShowAnimations();
        if (!showAnims) {
            DynamicAnim.disableAnimations();
        } else {
            DynamicAnim.enableAnimations();
        }
    }
    
    private void applyCardRounding(int roundingDp) {
    ...
    private void applyIconSize(int sizePercent) {
        // Icon size implementation would go here
    ...
    private void applyCompactMode(boolean enabled) {
        if (binding == null) return;
        
        if (enabled) {
            // Reduce padding/margins for compact mode
            int smallPadding = (int)(8 * getResources().getDisplayMetrics().density);
            if (binding.getRoot() instanceof ViewGroup) {
                ((ViewGroup) binding.getRoot()).setPadding(smallPadding, smallPadding, smallPadding, smallPadding);
            }
        } else {
            // Restore normal padding
            int normalPadding = (int)(16 * getResources().getDisplayMetrics().density);
            if (binding.getRoot() instanceof ViewGroup) {
                ((ViewGroup) binding.getRoot()).setPadding(normalPadding, normalPadding, normalPadding, normalPadding);
            }
        }
    }
    ...
    private void setupAccountHeader() {
        Button headerSignInButton = findViewById(R.id.header_sign_in_button);
        if (headerSignInButton != null) {
            headerSignInButton.setOnClickListener(v -> {
                Intent intent = new Intent(this, MsftLoginActivity.class);
                accountLoginLauncher.launch(intent);
            });
            DynamicAnim.applyPressScale(headerSignInButton);
        }

        TextView headerAccountName = findViewById(R.id.header_account_name);
        com.microsoft.xbox.idp.toolkit.CircleImageView headerAvatar = findViewById(R.id.header_account_avatar);
        
        // Update header account info when account changes
        refreshAccountHeaderUI();
    }
    ...
    private void refreshContentCounts() {
        ...
        org.chimeramc.launcher.settings.FeatureSettings.StorageType storageType;
        try {
            storageType = org.chimeramc.launcher.settings.FeatureSettings.StorageType.valueOf(savedType);
        } catch (IllegalArgumentException e) {
            storageType = org.chimeramc.launcher.settings.FeatureSettings.StorageType.INTERNAL;
        }
