...
final class BundledResourcePackInstaller {
    ...
    private static final String MANAGED_STATE = "chimeralauncher_bundled_mod_packs.json";
    private static final String LEGACY_STATE = "chimeralauncher_bundled_resource_packs.json";
    ...
    private static Pack readPack(String modId, File root, PackType type) {
        ...
        try (FileReader reader = new FileReader(manifest)) {
            ...
            String prefix = type == PackType.RESOURCE ? "chimeralauncher_rp_" : "chimeralauncher_bp_";
    ...
    private static void syncRoot(File gameDataDir, List<Pack> desired) throws IOException {
        ...
        for (Pack pack : desired) {
            ...
            File staging = new File(parent, pack.targetName + ".chimeralauncher_tmp");
    ...
    private static List<ManagedPack> readState(File stateFile) {
        ...
        try (FileReader reader = new FileReader(stateFile)) {
            ...
            for (JsonElement element : entries) {
                ...
                if (!target.startsWith("chimeralauncher_") || !target.equals(new File(target).getName())) continue;
    ...
    private static void writeJsonAtomic(File target, JsonElement json) throws IOException {
        ...
        File temp = new File(target.getParentFile(), target.getName() + ".chimeralauncher_tmp");
